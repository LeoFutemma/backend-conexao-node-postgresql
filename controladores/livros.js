...
const excluirLivro = async (req, res) => {
    ...
    try {
        const livro = await conexao.query('select * from livros where id = $1', [id]);
        const { rows: emprestimos } = await conexao.query('select * from emprestimos where livro_id = $1', [id]);
        ...
        if (emprestimos[0]) {
            return res.status(400).json('Não é possível excluir um livro que possui empréstimos atrelados ao seu ID');
        }
