...
const excluirAutor = async (req, res) => {
    ...
    try {
        const autor = await conexao.query('select * from autores where id = $1', [id]);
        const { rows: livros } = await conexao.query('select * from livros where autor_id = $1', [id]);
        ...
        if (livros[0]) {
            return res.status(400).json('Não é possível excluir um autor que possui livros atrelados ao seu ID');
        }
